...
class AdminCoachTests(unittest.TestCase):
    ...
    def test_coach_requests_pasted_thread_before_diagnosing_issue(self):
        provider=FakeProvider();repo=KnowledgeRepository(ROOT/"knowledge")
        coach=AdminAICoach(provider,repo,FakeTraining(),FakeIssues())
        result=coach.respond("Emma gave the wrong response",[])
        self.assertIn("copy and paste the complete conversation",result.message)
